...
\centering
\textbf{Flood Fill} --- distanza $d(x,y)$ dal goal:
...
\begin{minipage}{0.48\textwidth}
\centering
\textbf{Incremental A*} --- valore $f = g + h$:
...
img = mpimg.imread(str(Path("../logs/variant_plots/variant_combined.png")))
...
La @fig-variant analizza l'impatto della **revisit penalty** — un meccanismo che penalizza le celle già visitate per incentivare l'esplorazione di zone nuove. Le varianti "+" indicano la versione con penalty attiva.
